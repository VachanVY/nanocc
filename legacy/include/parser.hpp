...
class IfElseNode;
class CompoundNode; // Do we need this? Just holds a BlockNode... just use
                    // BlockNode directly?
class BreakNode;
class ContinueNode;
class WhileNode;
...
class ForInitNode; // Helper for init in ForNode | Can be declaration,
...
class ExprFactorNode : public ExprNode {
  /*everything which <factor> can parse has higher precedence than what <expr>
    can parse; eg. !a + b is parsed as [!a] + b not ![a + b] we keep function
    calls here so that the below doesn't happen: eg. b(x) + c is parsed as
    [[b(x)] + c] not [b][(x) + c]
  */
public:
  ...
  void parse(std::deque<Token>& tokens, size_t& pos);
  void dump(int indent = 0) const override;
  /// @brief runtime polymorphism for BinaryNode, AssignmentNode,
  /// ConditionalNode
...
public:
  ...
  std::shared_ptr<IRValNode> generateIR(
      std::vector<std::unique_ptr<IRInstructionNode>>& instructions) override;
};
...
public:
  ...
  AssignmentNode(std::unique_ptr<ExprNode> left,
                 std::unique_ptr<ExprNode> right)
...
public:
  ...
  ConditionalNode(std::unique_ptr<ExprNode> cond,
                  std::unique_ptr<ExprNode> t_expr,
