...
* fixUpInstructions methods for ASM nodes to fix instructions needing multiple
steps
*/
...
std::vector<std::unique_ptr<AsmInstructionNode>> IRUnaryNode::lowerToAsm() {
  ...
  if (this->op_type == "!") { // relational op
    ...
    auto cmp_instr = std::make_unique<AsmCmpNode>(
    ...
    throw std::runtime_error("IRUnaryNode::lowerToAsm: Unsupported op_type " +
                             this->op_type);
  }
}
...
std::vector<std::unique_ptr<AsmInstructionNode>> IRBinaryNode::lowerToAsm() {
  ...
  if (this->op_type == "/" || this->op_type == "%") {
    ...
    auto binary_instr =
        std::make_unique<AsmBinaryNode>(this->op_type, src2, dest);
    ...
    auto setcc_instr =
        std::make_unique<AsmSetCCNode>(getCondCode(this->op_type), dest);
    instructions.push_back(std::move(setcc_instr));
  } else {
    throw std::runtime_error("IRBinaryNode::lowerToAsm: Unsupported op_type " +
                             this->op_type);
  }
  return instructions;
}
...
std::shared_ptr<AsmStackNode> resolvePseudoRegister(
    AsmPseudoNode*
  ...
  if (assigned) {
    ...
    return std::make_shared<AsmStackNode>(
        -pseudo_reg_map[pseudo_src->identifier]);
  } else {
...
void AsmBinaryNode::resolvePseudoRegisters(
  ...
  if (auto pseudo_right = dyn_cast<AsmPseudoNode>(this->dest.get())) {
    this->dest =
        resolvePseudoRegister(pseudo_right, pseudo_reg_map, stack_offset);
  }
}
...
void AsmMovNode::fixUpInstructions(
  ...
  if (src_stack && dest_stack) {
    ...
    auto tmp_reg = std::make_shared<AsmRegisterNode>(
        getRegString(Reg::r10d)); // using %r10d as temporary
...
void AsmBinaryNode::fixUpInstructions(
  ...
  if ((this->op_type == "+" || this->op_type == "-") && src_is_stack &&
      dest_is_stack) {
    // Eg:
...
void AsmCmpNode::fixUpInstructions(
    std::vector<std::unique_ptr<AsmInstructionNode>>& instructions) {
  if (isa<AsmStackNode>(this->src1.get()) &&
      isa<AsmStackNode>(this->src2.get())) {
    auto tmp_reg = std::make_shared<AsmRegisterNode>(getRegString(Reg::r10d));
