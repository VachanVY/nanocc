...
void resolveVariableIdentifiers(IdentifierMap& identifier_map,
  ...
  if (identifier_map.contains(var_name->name) &&
    ...
    throw std::runtime_error(std::format(
        "Type Error: Redeclaration of parameter '{}'", var_name->name));
  }
  std::string unique_name = getUniqueName(var_name->name);
  identifier_map[var_name->name] = (VariableScope){
      unique_name, /*from_curr_scope=*/true, /*external_linkage=*/false};
...
linkage int main(void) { int foo(void); int foo(void); return foo();
}
...
void VarNode::resolveTypes(IdentifierMap& identifier_map) {
  if (identifier_map.contains(this->var_name->name)) {
    ...
    throw std::runtime_error(std::format("Type Error: Undeclared variable '{}'",
                                         this->var_name->name));
  }
}
...
void AssignmentNode::resolveTypes(IdentifierMap& identifier_map) {
  assert(this->left_expr && this->left_expr->left_exprf &&
         "Left expression or its factor is null in "
         "`AssignmentNode::resolveTypes`");
...
void FunctionCallNode::resolveTypes(IdentifierMap& identifier_map) {
  ...
  this->func_identifier->name =
      identifier_map[this->func_identifier->name].unique_name;
...
void BreakNode::checkTypes(TypeCheckerSymbolTable& type_checker_map) {
}; // no-op
void ContinueNode::checkTypes(TypeCheckerSymbolTable& type_checker_map) {
}; // no-op
...
void VarNode::checkTypes(TypeCheckerSymbolTable& type_checker_map) {
  if (!std::holds_alternative<IntType>(
          type_checker_map[this->var_name->name])) {
    throw std::runtime_error(
...
void NullNode::loopLabelling(std::string& loop_label) {
}; // no-op
   // loop labelling -- end
